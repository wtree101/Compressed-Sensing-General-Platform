function  [Error_Stand, Error_function] = onetrial_SubGD(m,r,kappa,lambda,params)
    %todo: add lambda
    d1 = params.d1;
    d2 = params.d2;
...
Ul = U0(:,1:r)*sqrt(S0(1:r,1:r));
    ...
    [X0,U0] = Initialization_random(y,A,d1,d2,r,m,params.init_scale);
    Ul = U0;
    Xl = X0;
end
...
% standard RGD
% Design a decaying schedule for mu (step size)
% Example: Exponential decay
mu0 = mu; % initial step size
decay_rate = 0.995; % decay factor per iteration

% In the main loop, update mu at each iteration:
% mu = mu0 * decay_rate^(l-1);
...
for l = 2:T
    ...
    Gl = (1/m) * reshape(A' * s, [d1, d2]);
    Ul = Ul + mu0 * (Gl * Ul - lambda * Ul);
    mu0 = mu0 * decay_rate; % Update step size
