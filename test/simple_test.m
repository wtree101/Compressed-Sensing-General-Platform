%%%%%%%%%% Simple Test Script for Low-rank Matrix Recovery
% Quick test with fixed parameters for debu% Plot convergence curves
subplot(1, 3, 1);
hold on;
colors = ['b', 'r', 'g', 'm', 'c', 'k'];
...
algorithms = [0, 1, 4,5]; % GD, RGD, PGD, AP (if available)
...
for alg_idx = 1:length(algorithms)
    ...
    if alg_flag == 4 % PGD algorithm
        ...
        params.mu = 0.01; % Smaller step size for PGD stability
    end
...
%% Save Test Results
save('simple_test_results.mat', 'results', 'T', 'd1', 'd2', 'm', 'r', 'r_star');

fprintf('\n=== Test Complete ===\n');
fprintf('Results saved to: simple_test_results.mat\n');
fprintf('Plots displayed for visual inspection\n');

%% Helper Function: Rank-r Projection
function X_proj = project_rank_r(X, r, ~, ~)
    % Project matrix X onto the set of rank-r matrices
    % Inputs:
    %   X - Input matrix to project
    %   r - Target rank
    %   ~ - Matrix dimensions (unused, for compatibility)
    % Output:
    %   X_proj - Projected matrix with rank at most r
